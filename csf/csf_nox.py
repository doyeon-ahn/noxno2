...
def _geoscf_filepath(collection, timestamp_utc, d_geoscf, mode="ana"):
    ...
    stamp = ts.strftime(f"%Y%m%d_{hh}{mm}z")
    fname = f"GEOS.cf.{mode}.{collection}.{stamp}.R0.nc4"
    return os.path.join(d_geoscf, mode, collection, ts.strftime("%Y/%m/%d"), fname)
...
def step1_pss_ratio(trop_csf, tdump, d_geoscf,
                    suffix="_O",
    ...
    """
    out = trop_csf.copy()
    pss_cols = ["pss_ratio", "jno2", "k_no_o3",
                "o3_molmol", "o3_nd", "T_K_geoscf", "cosz", "P_Pa"]
    for c in pss_cols:
        out[c] = np.nan
