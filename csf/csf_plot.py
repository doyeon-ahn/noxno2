...
def _plot_map(ax, CFG, target, data, data_sampled, data_true_sampled,
	...
	data.plot(ax=ax, cax=cax, column=voi, cmap=style["cmap"], alpha=1.0,
...
def _plot_csf(CFG, target, data, data_sampled, data_true_sampled, data_csf, tdump,
	...
	fig, ax = plt.subplots(nrows=7, ncols=11, figsize=(7.24, 4.71),
						   gridspec_kw=dict(width_ratios=[1]*11, height_ratios=[1]*7))
	plt.subplots_adjust(left=0.01, bottom=0.09, right=0.963, top=0.99, wspace=0, hspace=0)
	for j in range(7):
		for i in range(11):
			ax[j, i].axis("off")
