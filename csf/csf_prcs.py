...
CFG = {
	...
	"SATE_INFO":		 ["trop", CT.DATA_VER["trop"], CT.PRCS_VER["trop"], "2022-08-07", "2022-08-08", "qf_good", "prcsd"],
	"HYSTRAJ_RUN_VER":	 CT.PRCS_VER["hystraj"],
	"o_calc_nox":		 False,
	"o_plot":			 [True, 20],	# [True/False; Every X overpasses]
	"snapshot":			 True,
	"CEMS_LOOKBACK_HRS": 6,
}
...
target =				 target.loc[target["facilityId"].isin(["6481"])].reset_index(drop=True)
...
def _csf_prcs(CFG, target):
	...
	for it in range(len(target)):
		...
		for i_sate in range(len(sate)):
			...
			nox_result = run_nox_workflow(	trop_csf=trop_csf, true_tdump=true_tdump, d_geoscf=CT.d_geoscf, suffix="_O", option_b_or_a="B",)
