...
def _scatter_qf(ax, data_csf, age_col, val_col, color, marker="o", label=None):
	...
	if qf in data_csf.columns:
		...
		ax.scatter(data_csf[age_col], data_csf[val_col], marker=marker, color=color, label=label, zorder=5)
...
def _plot_map(ax, CFG, target, data, data_sampled, data_true_sampled,
	...
	data.plot(ax=ax, column=voi, cmap=style["cmap"], alpha=1.0,
			  vmin=lo, vmax=hi, legend=True,
			  legend_kwds={"label": style["label"], "orientation": "horizontal", "shrink": 0.5, "pad": 0.03})
	...
	ax.plot(tdump["longitude"].values, tdump["latitude"].values,
			color="white", lw=0.8, linestyle="solid", marker="o", markersize=0.1)
...
def _plot_gaussian_panels(ax, pp_list, CFG, data_csf_H, data_sampled, style):
	voi		 = style["voi"]
	flux_var = style["flux_var"]
	xlim	 = [-110., 110.]

	all_vals = pd.concat([data_sampled.loc[data_sampled["tdump_id"] == tid, voi]
						  for tid in data_csf_H["tdump_id"]])
	ylim = style["ylim_pad"](all_vals.min(), all_vals.max())

	for i, pp in enumerate(pp_list[:len(data_csf_H)]):
		row = data_csf_H.loc[i]
		ax[pp].axis("on")
		is_last = (pp == pp_list[min(len(data_csf_H) - 1, len(pp_list) - 1)])
		...
		qf_pass = "QF_gauss_abs" in data_csf_H.columns and row["QF_gauss_abs"] == 0
		clr		= "limegreen" if qf_pass else "grey"
		...
		if not np.isnan(row[flux_var]):
			for k, (xpos, txt) in enumerate([
				...
				ax[pp].text(xpos, 1.0 - 0.105 * (k + 1), txt,
							transform=ax[pp].transAxes, fontsize=4.0,
							color="black", ha="left")
...
def _plot_flux_vs_time(ax, CFG, data_csf, trop_csf, nox_result, cems_nox, flux_unit, style):
	"""
	...
	if trop_csf is not None and "flux_nox_fit" in trop_csf.columns:
		fit = trop_csf[[age_O, "flux_nox_fit"]].dropna().sort_values(age_O)
		if not fit.empty:
			ax.plot(fit[age_O], fit["flux_nox_fit"],
	...
	if CFG["SATE_INFO"][0] == "trop" and CFG.get("o_calc_nox"):
		_scatter_qf(ax, data_csf, age_H, "flux_nox_H", "orangered", label="NOx flux (HYSPLIT)")
		x = np.arange(0., 6., 0.1)
...
def _plot_csf(CFG, target, data, data_sampled, data_true_sampled, data_csf, tdump,
	...
	fig, ax = plt.subplots(nrows=7, ncols=11, figsize=(7.24, 4.71), gridspec_kw=dict(width_ratios=[1]*11, height_ratios=[1]*7))
	plt.subplots_adjust(left=0.01, bottom=0.09, right=0.920, top=0.99, wspace=0, hspace=0)
	for j in range(7):
		for i in range(11):
			ax[j, i].axis("off")
	...
	ax_flux = fig.add_subplot(gs[5:8, 1:11])
	pp_list = [(r, c) for r in range(4) for c in range(3, 11)]	 # 4×8 = 32 Gaussian slots
