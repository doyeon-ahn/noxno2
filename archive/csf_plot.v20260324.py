...
import matplotlib.pyplot as plt  # ← move pyplot import HERE, immediately after use()
import contextily as ctx
import matplotlib.cm as cmx

# NOW import everything else (including IPython)
...
def _plot_flux_vs_time(axbig2, CFG, data_csf, cems_nox, flux_unit, style):
	...
	# ------------------------------------------------------------------ #
	# Secondary x-axis: age_km, derived from age_hours via linear interp  #
	# ------------------------------------------------------------------ #
	age_km_col = "age_km_H" if "age_km_H" in data_csf.columns else "age_km"

	if age_km_col in data_csf.columns and age_H in data_csf.columns:
		# Build a monotonic hours→km interpolator from the HYSPLIT trajectory
		_df = (data_csf[[age_H, age_km_col]]
			   .dropna()
			   .drop_duplicates(subset=age_H)
			   .sort_values(age_H))

		hours_arr = _df[age_H].values
		km_arr	  = _df[age_km_col].values

		def _hours_to_km(h):
			"""Map bottom-axis hour ticks → km labels via linear interp."""
			return np.interp(h, hours_arr, km_arr)

		def _km_to_hours(k):
			"""Inverse map for twin axis (matplotlib needs both directions)."""
			return np.interp(k, km_arr, hours_arr)

		ax_km = axbig2.secondary_xaxis("top",
										functions=(_hours_to_km, _km_to_hours))
		ax_km.set_xlabel("Transport distance [km]")
		ax_km.xaxis.set_major_formatter(plt.FuncFormatter(lambda x, _: f"{x:.0f}"))

	# NOx/NO2 ratio at PSS
	IPython.embed()
	_scatter_qf(age_H, 'pss_ratio', "purple", label='NOX/NO2 (PSS)')
	...
	if cems_nox is not None and not cems_nox.empty:
		...
		axbig2.step(cems_nox["transport_hrs"], cems_nox["noxMass_tph_metric"], where="mid", color="darkorange", lw=1.2, label="CEMS NOx")
		axbig2.scatter(cems_nox["transport_hrs"], cems_nox["noxMass_tph_metric"], marker="s", color="darkorange", s=18, zorder=5)
		axbig2.text(0.02, 0.92, f"CEMS mean= {cems_nox['noxMass_tph_metric'].mean():.2f} {flux_unit}", transform=axbig2.transAxes, fontsize=7, color="darkorange")
