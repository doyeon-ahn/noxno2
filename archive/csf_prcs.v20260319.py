...
CFG = {
	...
	"SATE_INFO":		["trop", CT.DATA_VER["trop"], CT.PRCS_VER["trop"], "2022-10-13", "2022-10-14", "qf_good", "prcsd"],
...
def _process_overpass(it, i_sate, target, sate, tname, tid, dout_csv, dout_png):
	...
	if not cems_nox.empty:
		print(f"  [CEMS] {len(cems_nox)} hourly records, " f"mean NOx = {cems_nox['noxMass_tph_metric'].mean():.2f} tNOx/hr (metric)")
		print (cems_nox)


	if CFG["o_plot"]:
		_plot_csf(CFG, target=target.loc[it], data=trop, data_sampled=trop_sampled,
				  data_csf=trop_csf, tdump=tdump, sate=row,
				  flux_unit="[tNO2/hr]", o_plot_wddiff=False, dfout=dout_png+fout, cems_nox=cems_nox)


def csf_prcs(CFG, target):
