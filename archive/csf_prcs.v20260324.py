...
CFG = {
	...
	#"SATE_INFO":		 ["trop", CT.DATA_VER["trop"], CT.PRCS_VER["trop"], "2009-01-01", "2025-12-31", "qf_good", "prcsd"],
	"SATE_INFO":		 ["trop", CT.DATA_VER["trop"], CT.PRCS_VER["trop"], "2025-08-22", "2025-08-23", "qf_good", "prcsd"],
	"HYSTRAJ_RUN_VER":	 CT.PRCS_VER["hystraj"],
	"o_calc_nox":		 False,
	"o_plot":			 [True, 1],		# [True/False; Every X overpasses]
	"snapshot":			 True,
	"CEMS_LOOKBACK_HRS": 6,
}
...
target =				 target.loc[target["facilityId"].isin(["6076"])].reset_index(drop=True)
...
def _read_tdump(satellite, tname_long, tid, time):
	...
	# Cumulative distance (km) traveled from origin to each coordinate
...
def _tag_tdump_times(tdump, time_utc_overpass):
	"""
	Attach a 'time_tag' column to tdump:
	  tdump_id == 0  →	time_utc_overpass	(satellite observation time at source)
	  tdump_id >  0  →	time_utc_overpass - age_hours  (back-transport time)

	'time_tag' is a tz-naive UTC Timestamp that can be used directly to
	look up CEMS emission records corresponding to when the air parcel was
	at each trajectory segment location.

	Parameters
	----------
	tdump : pd.DataFrame   output of _read_tdump
	time_utc_overpass : str | pd.Timestamp
		TROPOMI overpass time for this file (the 'time_utc_i' column from the
		satellite CSV index, or equivalent).

	Returns
	-------
	tdump : pd.DataFrame   same frame with 'time_tag' column added (in-place copy)
	"""
	t0 = pd.Timestamp(time_utc_overpass)
	if t0.tzinfo is not None:
		t0 = t0.tz_convert("UTC").tz_localize(None)
	# age_hours in HYSPLIT backward runs is negative (back in time)
	# Use the sign as-is so that subtraction always moves backward.
...
def _read_trop(dfin, tname_long, sate_row):
	...
	Scanline / pixel subsets are taken from the CSV index row (sate_row)
	when those columns are present and non-null.
	"""
	var_list = [
...
def _calc_qf_gauss(csf, suffix=""):
	"""
	Assign an absolute Gaussian quality flag (QF_gauss_abs) to each CSF row.

	QF_gauss_abs = 0  →  good	(all criteria pass)
	QF_gauss_abs = 1  →  bad	(any criterion fails)

	Criteria (column names use the supplied suffix, e.g. "_H"):
		rsq_detrend  >= 0.5		detrended R² — fit explains variance above baseline
		d_gap		 <= 50 %	no large sampling gap within the FWHM
		d_left		 >= -20 %	plume extends adequately to the left
		d_right		 >= -20 %	plume extends adequately to the right
		d_center	 <= 30 %	peak centre is well-sampled
		a2sigpct	 <= 50 %	amplitude uncertainty < 50 % of amplitude
		a4sigpct	 <= 50 %	width uncertainty < 50 % of width
		nobs		 >= 5		minimum number of satellite pixels

	Parameters
	----------
	csf    : pd.DataFrame  output of _calc_csf (columns may carry suffix)
	suffix : str		   column suffix, e.g. "_H" or "" (plain)

	Returns
	-------
	csf    : pd.DataFrame  with new column	QF_gauss_abs{suffix}
	"""
	s	= suffix
	out = csf.copy()
...
def _build_true_tdump(data_csf, tdump):
	"""
	Build 'true' trajectory by connecting a3 lat/lon coordinates and compute
	true wind-speed components from adjacent a3 distances.
	Returns a tdump-compatible DataFrame, or empty DataFrame if < 2 valid a3 points.
	The 'time_tag' column is preserved from the original tdump so that
	CEMS look-up times stay consistent after optimisation.
	"""
	...
	true = pd.DataFrame({
		...
		"time_tag":  base["time_tag"].values,	# ← carry backward-time tags forward
...
def _read_cems_nox(tid, tname, time_utc):
	...
	time_utc may be a plain Timestamp (no tz) or tz-aware; both are handled.
	Returns a DataFrame with columns: [tstmp_utc, noxMass_tph, noxMass_tph_metric].
	...
	hourly["noxMass_tph_metric"] = hourly["noxMass_tph"] * 0.000453592	# lbs/hr → metric t/hr
	return hourly
# =============================================================================
...
def _calc_csf(satellite, target, data, constraint, tdump):
	...
	'time_tag' (backward-transport UTC time) is merged in from tdump so that
	every flux row carries the time at which the air parcel was at that
	trajectory location — used later for CEMS matching.
	"""
	merge_cols = ["tdump_id", "longitude", "latitude", "age_hours",
				  "month", "hour", "temp", "pres", "u", "v", "wso", "wd",
				  "time_tag"]	# ← include time_tag
	...
	for i in range(len(out)):
		...
		# flux = √(π/ln2) × 0.5 × (amplitude × width) × wind_speed
		if satellite == "trop":
			out.loc[i, "flux_no2"] = (
				0.5 * (np.pi / np.log(2)) ** 0.5	# Area under the Gaussian curve = 0.5 * sqrt(pi / ln2) * a2 * a4 
...
# =============================================================================
# SATELLITE FILE COLLECTION  (reads CSV index — no .nc I/O here)
# =============================================================================
def _collect_satellite_files(tname):
	"""
	Return a filtered DataFrame of TROPOMI overpasses for the target.
	Reads the pre-built CSV index file instead of opening individual .nc files.
	'time_utc_i' (start of overpass) is used for date-range filtering and
	as the reference observation time for trajectory time-tagging.
	'file' should be the bare filename (without directory); the full path is
	reconstructed from CFG["SATE_INFO"].
	"""
	csv_path = os.path.join(CT.d_noxno2, 'd_dat', 'trop', CFG['SATE_INFO'][2], 'd_csv', tname+'_'+CFG['SATE_INFO'][2]+'.csv')
	if not os.path.isfile(csv_path):
		raise FileNotFoundError(f"[_collect_satellite_files] No CSV index found for {tname}\n")
	sate = pd.read_csv(csv_path)
	# Parse as UTC then strip tzinfo → tz-naive UTC, consistent with CFG date strings
	...
	# Use time_utc_i as the canonical observation time (consistent with original code)
	sate["time_utc"] = sate["time_utc_i"]
	# Reconstruct full .nc path from the 'file' column
...
# =============================================================================
# MAIN PROCESSING FUNCTION	(linear flow — no nested helpers)
# =============================================================================
def _csf_prcs(CFG, target):
	"""
	Main entry point.  For each target facility and each TROPOMI overpass:
	  1. Collect satellite index rows			(_collect_satellite_files)
	  2. For each overpass row:
		 a. Read TROPOMI pixels					(_read_trop)
		 b. Read HYSPLIT back-trajectory		(_read_tdump)
		 c. Tag trajectory with backward times	(_tag_tdump_times)
		 d. Sample pixels along trajectory		(_sample_soundings_along_traj)
		 e. Fit Gaussian & compute HYSPLIT CSF	(_calc_csf)			 → _H suffix
		 f. Build optimised trajectory			(_build_true_tdump)
		 g. Re-sample on optimised trajectory	(_sample_soundings_along_traj)
		 h. Fit Gaussian & compute optimised CSF(_calc_csf)			 → _O suffix
		 i. Merge _H and _O results; save CSV
		 j. Read & save CEMS NOx			   (_read_cems_nox)
		 k. Plot								(_plot_csf)
	"""
	for it in range(len(target)):
		...
		# ------------------------------------------------------------------
		# 1. Collect satellite overpasses from CSV index
		# ------------------------------------------------------------------
		...
		for i_sate in range(len(sate)):
			...
			# --------------------------------------------------------------
			# 2a. Read HYSPLIT back-trajectory
			# --------------------------------------------------------------
			...
			# --------------------------------------------------------------
			# 2c. Tag each trajectory point with its backward UTC time
			#	  tdump_id == 0  →	satellite observation time	(time_utc_i)
			#	  tdump_id >  0  →	time_utc_i - |age_hours|
			#	  Use 'time_tag' to look up the matching CEMS emission rate.
			# --------------------------------------------------------------
			tdump = _tag_tdump_times(tdump, sate_row["time_utc"])

			# --------------------------------------------------------------
			# 2d. Sample TROPOMI pixels along HYSPLIT trajectory
			# --------------------------------------------------------------
			...
			# --------------------------------------------------------------
			# 2e. First-pass CSF using HYSPLIT trajectory  (_H suffix)
			# --------------------------------------------------------------
			...
			# after step 2h (inside the `if len(trop_true_sampled) > 0:` block) ← insert here


			# --------------------------------------------------------------
			# 2f. Build optimised (true) trajectory from Gaussian peak locs
			# --------------------------------------------------------------
			true_tdump = _build_true_tdump(trop_csf.rename(columns=lambda c: c.replace("_H", "")), tdump)

			# --------------------------------------------------------------
			# 2g-h. Optimised CSF  (_O suffix) — only if enough a3 points
			# --------------------------------------------------------------
			...
			# --------------------------------------------------------------
			# 2i. Save merged CSF result
			#	  Output contains 'time_tag_H' (and 'time_tag_O') so that
			#	  every flux row can be matched to the correct CEMS hour.
			# --------------------------------------------------------------
			trop_csf.to_csv(dout_csv + fout + ".csv", index=False)
			trop_csf, nox_result = run_nox_workflow(	trop_csf=trop_csf, true_tdump=true_tdump, d_geoscf=CT.d_geoscf, suffix="_H", option_b_or_a="A",)

			if CFG["o_plot"][0] == True and i_sate % CFG['o_plot'][1] == 0 and len(trop_csf) > 1 and len(trop_true_sampled) > 1:
				# --------------------------------------------------------------
				# 2j. CEMS NOx — look up using the overpass observation time
				#	  (time_utc_i).  For trajectory-matched CEMS look-up use
				#	  each row's 'time_tag_H' / 'time_tag_O' from the saved CSV.
				# --------------------------------------------------------------
				cems_nox = _read_cems_nox(tid, tname, sate_row["time_utc"])
				if not cems_nox.empty:
					print(f"  [CEMS] {len(cems_nox)} hourly records, mean NOx = {cems_nox['noxMass_tph_metric'].mean():.2f} tNOx/hr (metric)")
				...
				_plot_csf(
					...
					data=trop, data_sampled=trop_sampled,
					data_true_sampled=trop_true_sampled,
					data_csf=trop_csf, tdump=tdump,
					sate=sate_row, flux_unit="[tNO2/hr]",
